...
The animated banner loads a bundled `file://` page; it does not start a server. On macOS, the plugin and native companion communicate through a `0700` temporary directory with `0600` request files. Reminder data is encoded into the local URL, and only local GIF/WebP files accepted by the plugin are displayed.
